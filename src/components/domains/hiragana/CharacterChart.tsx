    ...
    <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2 md:gap-3">
      {data.map((item, index) => (
        <motion.div
          ...
          whileTap={{ scale: 0.95 }}
        >
          <Card className="aspect-square cursor-pointer hover:shadow-lg transition-all duration-200 bg-white/90 backdrop-blur-sm border-2 hover:border-pink-300">
            <CardContent className="p-1.5 md:p-2 flex flex-col items-center justify-center h-full">
              ...
              <div className="text-xs text-gray-600 text-center space-y-0.5">
                ...
                <div className="text-pink-600 font-semibold text-xs">{item.korean}</div>
              </div>
            </CardContent>
          </Card>
        </motion.div>
